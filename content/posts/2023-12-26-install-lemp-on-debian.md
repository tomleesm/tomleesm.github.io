...
server {
  ...
  root  /home/tom/apps/localhost/current/public;
  location / {
...
``` bash
sudo ln -s /etc/nginx/sites-available/localhost.conf \
...
編輯 `info.php` 檔案
``` bash
vim ~/apps/localhost/current/public/info.php
```
複製貼上以下程式碼
...
``` bash
rm ~/apps/localhost/current/public/info.php
```
## 前端編譯用程式
...
vim ~/.bash_profile
加上 export PATH=~/.local/bin:~/.config/composer/vendor/bin:$PATH # 等號兩邊不要加空格
source ~/.bash_profile
```
